...
public class transaksi extends user{
    private float jumlah, saldo_tabungan, bunga;
    ...
    void TarikTunai(){
        try{
        System.out.println("=======================================================");
        System.out.println("Minimal pengambilan uang tunai sebesar Rp. 50,000");
    ...
    void addDeposito(){
        try{
        System.out.println("=======================================================");
        System.out.println("Minimal Deposit sebesar Rp. 100,000");
        System.out.println("Terdapat Bunga sebesar 10% apabila memasukkan deposit sebesar Rp. 5,000,000");
        System.out.print("Jumlah Uang Deposit Yang Ingin Dimasukkan : Rp. ");
        jumlah=sc.nextFloat();
        Class.forName("org.sqlite.JDBC");
    Connection k = DriverManager.getConnection("jdbc:sqlite:D:/OOP ATM/atm.db");
    Statement stat = k.createStatement();
        if(jumlah>=5000000 && jumlah>=100000){
    bunga=jumlah*0.1f;
    jumlah=jumlah+bunga;
    ...
    System.out.println("Anda mendapatkan bunga sejumlah Rp."+String.format("%,.2f",(bunga)));
    System.out.println("Anda memasukkan deposit sejumlah Rp. "+String.format("%,.2f",jumlah));
} else if(jumlah>=100000){
    stat.executeUpdate("update rekening set saldo_deposit=saldo_deposit+"+jumlah+" where no_rek='"+getNo_rek()+"';");
    stat.executeUpdate("insert into transaksi values('"+getNo_rek()+"', 'Setor Deposit', date('now'), "+jumlah+",'"+getNo_rek()+"');");
    System.out.println("Anda memasukkan deposit sejumlah Rp. "+String.format("%,.2f",jumlah));
} else {System.out.println("Uang deposit yang ingin dimasukkan kurang dari Rp. 100,000");}
} catch (Exception e){System.out.println("Error: "+e.getMessage());}
    }
    ...
    void addTabungan(){
        try{
        System.out.println("=======================================================");
        System.out.println("Minimal setor tunai sebesar Rp. 5,000");
...
} else {System.out.println("Jumlah Uang Yang Anda Ketik Kurang dari Rp. 5,000");}
    }catch (Exception e){System.out.println("Error: "+e.getMessage());}
    }
